...
const ViewBookDetails = () => {
    ...
    useEffect(() => {
        const fetch = async () => {
            try {
                const response = await axios.get(`http://localhost:5002/api/v1/get-book-by-id/${id}`);
    ...
    const handleFavorite = async () => {
        try {
            const response = await axios.put("http://localhost:5002/api/v1/add-book-to-favourite", {}, { headers });
    ...
    const handleCart = async () => {
        try {
            const response = await axios.put("http://localhost:5002/api/v1/add-to-cart", {}, { headers });
            alert(response.data.message);
        } catch (error) {
    ...
    const deleteBook = async () => {
        try {
            const response = await axios.delete(`http://localhost:5002/api/v1/delete-book/`, { headers });
            alert(response.data.message);
            navigate('/all-books'); 
        } catch (error) {
            ...
            {Data && (
                <>
                    ...
                    <div className='p-4 w-3/6'>
                        ...
                        <p className="mt-4 text-zinc-900 text-3xl font-semibold">Price: RS {Data.price}{" "}</p>
                    </div>
                </>
            )}
        </div>
