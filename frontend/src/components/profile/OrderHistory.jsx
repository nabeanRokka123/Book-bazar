    ...
    const fetchOrderHistory = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5002/api/v1/get-order-history",
          { headers }
        );
    ...
    <div className="min-h-screen">
      {orderHistory && orderHistory.length > 0 && (
        <div className="min-h-screen p-0 md:p-4 text-zinc-100">
          <h1 className="text-xl md:text-xl font-semibold text-black mb-8">
            Order History
          </h1>
          ...
          {orderHistory.map((item, index) => (
            <div
              className="bg-zinc-800 w-full rounded py-2 px-4 flex gap-4 border border-white"
              key={index}
            >
              <div className="w-1/12">
