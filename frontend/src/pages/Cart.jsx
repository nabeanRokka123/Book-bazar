...
const Cart = () => {
    ...
    useEffect(() => {
        const fetchCart = async () => {
            try {
                const response = await axios.get("http://localhost:5002/api/v1/get-user-cart", { headers });
    ...
    const deleteItem = async (bookid) => {
        try {
            const response = await axios.put(`http://localhost:5002/api/v1/remove-from-cart/${bookid}`, {}, { headers });
            console.log(response);
    ...
    const placeOrder = async () => {
        try {
            const response = await axios.post(`http://localhost:5002/api/v1/place-order`, {order: cart}, { headers });
            ...
            {cart.length > 0 ? (
                <>
                    {cart.map((item, i) => (
                        <div className='w-full my-4 p-4 rounded-lg flex flex-col bg-zinc-800 justify-between items-center md:flex-row' key={i}>
                            ...
                            <div className='flex mt-4 w-full md:w-auto items-center justify-between'>
                                <h2 className='text-zinc-100 text-2xl font-semibold flex'>Rs {item.price}</h2>
                    ...
                    <div className='w-full mt-8 p-4 rounded-lg flex justify-end bg-zinc-800'>
                        <h2 className='text-3xl text-zinc-100 font-semibold'>Total: Rs {total}</h2>
                    </div>
